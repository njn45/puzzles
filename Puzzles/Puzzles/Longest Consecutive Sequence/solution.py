...
class Solution:
    ...
    def longest_consecutive(nums:list[int]) -> int:
        ans, nums = 0, set(nums) # use a set() for fast lookups

        for i in nums:
            if i-1 not in nums: # Find the start of a sequence
                current = 1
                while i + current in nums: # increment the sequence
                    current += 1 # current is essentially the length of the sequence
                ans = max(ans,current) # is this the longest sequence we've seen so far?

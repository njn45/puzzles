...
class Solution:
    ...
    def replace_elements(arr:list[int]) -> list[int]:
        """the trick with this is to go through the array backwards and just keep track of the maximum value"""

        cur_max = -1
        for i in range(len(arr)-1, -1, -1):
            new_max = max(cur_max, arr[i])
            arr[i] = cur_max # not new max to account for the end of the list always being -1
            cur_max = new_max

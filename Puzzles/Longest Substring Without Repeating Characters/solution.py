...
class Solution:
    ...
    def length_of_longest_substring(s:str) -> int:
        ...
        chars, left_idx, result = set(), 0, 0

        for right_idx in range(len(s)):
            while s[right_idx] in chars: # if we see a duplicate character
                chars.remove(s[left_idx]) # update sliding window by removing the left-most character
                left_idx += 1 # update positition of left pointer
            chars.add(s[right_idx]) # add the new character to our set
